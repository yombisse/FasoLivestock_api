...
<div class="alert alert-warning">
    Création de mouvements désactivée : les mouvements doivent être générés via les observateurs / autres modules pour garantir la traçabilité.
</div>
@endsection
...
@section('content')
<div x-data="mouvementCreateForm()" class="form-page fade-in">

    <form id="mouvement-create-form"
          method="POST"
          action="{{ route('admin.mouvements.store') }}">
        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-with-icon">
                                <select id="type_evenement_id"
                                    ...
                                    <option value="ACHAT" {{ old('type_evenement_id') === 'ACHAT' ? 'selected' : '' }}>Achat</option>
                                    <option value="VENTE" {{ old('type_evenement_id') === 'VENTE' ? 'selected' : '' }}>Vente</option>
                                    <option value="DECES" {{ old('type_evenement_id') === 'DECES' ? 'selected' : '' }}>Décès</option>
                                    <option value="PERTE" {{ old('type_evenement_id') === 'PERTE' ? 'selected' : '' }}>Perte</option>
                                    <option value="TRANSFERT" {{ old('type_evenement_id') === 'TRANSFERT' ? 'selected' : '' }}>Transfert</option>
                                    <option value="ABATTAGE" {{ old('type_evenement_id') === 'ABATTAGE' ? 'selected' : '' }}>Abattage</option>
                                </select>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <textarea id="description"
                                      ...
                                      rows="3"
                                      placeholder="Description du mouvement...">{{ old('description') }}</textarea>
                            @error('description')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
...
@push('scripts')
    <script src="{{ asset('admin/js/mouvements/create.js') }}"></script>
@endpush
